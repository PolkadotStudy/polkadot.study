...
Before we can start making Kitties, we need to do some groundwork. This part covers the patterns involved 
...
case we get lost is the change.

In the following command I'm naming the branch kitties:
...
As you can see in the image, the compilation should complete with a line that looks like this:
...
`do_something` stores a `u32` value in the runtime storage. Try setting a value and submiting the extrinsic.
...
extrinsic you called looks like. In the directory where you compiled the node go to  check the 

...
the same way the `template-pallet` is added. Notice kitties pallet is also being added to the features sections.
...
```
- Return to root directory. Compile and run it to check that everything is working as expected: return to the root folder and run:

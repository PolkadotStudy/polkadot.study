...
...
```
- Add the `pallet-kitties` to the runtime. Go to `./runtime/cargo.toml` file and add the pallet-kitties in 
...
```
- Go to `./runtime/src/lib.rs` and add the `pallet-kitties` in the same way the `template-pallet` is added.

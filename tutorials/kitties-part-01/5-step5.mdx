...
an RPC endpoints to communicate with a Substrate node. This allows us to use it to read storage items, and make extrinsics by calling our 
...
1. Kitties.js: this will render the Kitty panel, and contains the logics of fetching all kitties information from the connecting Substrate 
node.
...
We'll use api.query.templateModule.countForKitties to listen for a change in the amount of Kitties, which will query countForKitties 
from our Kitties pallet storage item. Then, we'll use the entries() method from Polkadot-JS API to get Kitty IDs and transform them with 
the getDna function.
...
// useEffect to fetch and set the kitty DNAs
  useEffect( () => {
      ...
      api.query.templateModule.countForKitties(async cnt => { 
        const entries = await api.query.templateModule.kitties.entries(),
...
// useEffect to subscribe to and update the kitties
  useEffect( () => {
    ...
    api.query.templateModule.kitties
...
Congratulations! We have setup the ground work of accessing the chain and saving all Kitty information internally using React. We'll come 
back to the Kitties.js component later once we create all the missing components of our application to complete it.

????????? should I add her the return function?

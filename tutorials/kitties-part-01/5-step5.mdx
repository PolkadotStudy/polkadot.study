...
It is important to understand the pallet design decisions we'll be making.

We will be breaking up the "heavy lifting" logic into private helper functions. This improves code 
...
* Checks if the kitty does not already exist in our storage map.
...
In order to use events inside our pallet, we add the associated type `RuntimeEvent` inside our pallet's 

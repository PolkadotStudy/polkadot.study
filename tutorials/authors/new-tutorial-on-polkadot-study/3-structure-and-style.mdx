...
Most likely you will like to add images to your tutorial. It is best to put them in a subfolder of your main tutorial repository.
The name of the folder should be `/assets`. When you copied the tutorial template, you will already have that folder.
...
:::info
Javascript code only works in `.mdx` files not in `.md` files, so make sure to name your tutorial file accordingly.
:::
...
Tasks are interactive elements, that ask the user to check a box. When a user wants to proceed to the next part of the tutorial
and not all *mandatory* tasks on the page are checked, the next button will be disabled and instead take them to the first unfinished task
they need to do before proceeding.
...
The Task component is still being worked on, the goal is to have a UI flow like [here](https://www.apollographql.com/tutorials/lift-off-part1/01-feature-overview-and-setup)

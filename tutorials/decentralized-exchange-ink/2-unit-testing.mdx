...
**Hint:** There are many websites that provide examples of testing ink! smart contracts using the following code: `ink-experimental-engine = ["ink_env/ink-experimental-engine"]`. However, it is important to note that if you come across such examples, they are **not up to date**.

To initiate tests execution, utilize the command `cargo test`.

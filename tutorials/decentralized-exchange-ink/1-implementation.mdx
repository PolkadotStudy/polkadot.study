...
ink! smart contract programming language is based on Rust and that is why the most important file where we will implement the whole business logic in `lib.rs`. Helicopter view of the file `lib.rs` looks like below:
...
Let's start from crucial subject in any decentralized exchange which is liquidity, below we can function responsible for providing liquidity:
...
`provide_liquidity` in `ink!` smart contract programming language every public function has to have `#[ink(message)]` attribute. There need to be at least one public method in a smart contract.

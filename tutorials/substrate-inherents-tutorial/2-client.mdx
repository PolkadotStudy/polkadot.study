...
data to Runtime upon authoring a new block.
An ```InherentDataProvider``` trait implementor utilizes ```provide_inherent_data``` method to put data onto 
storage location under a key both the Client and the Runtime share.
...
Inherent data provider instances are created by a closure passed to consensus block production module.
...
We see that there are already two inherent data providers returned by the closure which

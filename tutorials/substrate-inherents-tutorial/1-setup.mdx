...
A term extrinsic refers to a transaction in Substrate parlance.
End users usually submit signed transactions to be executed and stored on a block chain.
Substrate offers a functionality for submitting also unsigned extrinsics on chain.
This kind of transaction is in use, for example, by Offchain Workers that usually implement buisiness logic requiring transaction execution.
...
are not gossiped to the rest of the node for consensus, rather they are included into the block to be executed when the block is imported by the nodes.

...
named `1-structure-and-style.mdx`. Have a look
[in the repo](https://github.com/PolkadotStudy/website/tree/staging/docs/authors/new-tutorial-on-polkadot-study)
...
- **Step 2 / Subtopic 2 — Doing the Next Thing or explaining the next thing**
  (Level 2 heading)
- …
...
- Comprehensive and written for the `level` you will define in
  [Metadata](metadata)
...
- see [Codeblocks in the Component Cheatsheet](components-cheatsheet) to see how
  to include filenames, highlights and formatting.
...
Javascript code only works in `.mdx` files not in `.md` files, so make sure to

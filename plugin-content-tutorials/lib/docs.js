...
exports.createDocsByIdIndex =
  exports.toCategoryIndexMatcherParam =
  exports.isCategoryIndex =
  exports.getMainDocId =
  exports.addDocNavigation =
  exports.processDocMetadata =
  exports.readVersionDocs =
  exports.readDocFile =
    void 0;
...
async function readDocFile(versionMetadata, source) {
  const contentPath = await (0, utils_1.getFolderContainingFile)(
    (0, utils_1.getContentPathList)(versionMetadata),
    source
  );
...
async function doProcessDocMetadata({
  ...
  const {
    ...
    contentTitle,
    excerpt,
  } = await (0, utils_1.parseMarkdownFile)({
    filePath,
    fileContent: content,
    parseFrontMatter,
  });
  const frontMatter = (0, frontMatter_1.validateDocFrontMatter)(
    unsafeFrontMatter
  );
  const {
  ...
  const sourceFileNameWithoutExtension = path_1.default.basename(
    source,
    path_1.default.extname(source)
  );
  ...
  const level = frontMatter.level ?? "easy";
  ...
  function getDocEditUrl() {
    ...
    if (typeof options.editUrl === "function") {
      return options.editUrl({
        ...
        versionDocsDirPath: (0, utils_1.posixPath)(
          path_1.default.relative(siteDir, versionMetadata.contentPath)
        ),
  ...
  const formatDate = (locale, date, calendar) => {
    try {
      ...
      logger_1.default.error`Can't format docs lastUpdatedAt date "${String(
        date
      )}"`;
      throw err;
    }
  ...
  return {
    ...
    tags: (0, utils_1.normalizeFrontMatterTags)(
